...
| **`setUIMode` による UI モード** | **2**: `normal`（通常表示） / `focus`（ミニマル・既定）。 |
...
| 層 | 識別子・属性 | 役割 | 主な実装参照 |
|----|--------------|------|----------------|
| UI モード | `data-ui-mode="normal"` \| `focus` | 通常表示 / ミニマル | [`js/app.js`](../js/app.js) `setUIMode` |
...
2. **Normal（通常表示）**: 右サイドバーの **先頭操作帯** と **6 カテゴリのガジェット**。左ドックは **補助ワークスペース**（常時必須ではない）。
