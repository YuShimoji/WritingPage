...
> 最終更新: 2026-06-25 / v0.3.35（WP-005 MD preview rich-preview activation）
...
- **WP-005（session 94 起票）**: プレビュー・比較ツール再設計。スライスA は done: 分割ビュー比較の公開入口を structure sidebar / Electron menu から外し、command palette 比較コマンドを隠した。スライスB も done: MD プレビューを実表示の rich-preview surface として戻し、typing / scroll controller activation を focused E2E で固定した。次はスライスC: 比較ツール (chapter-compare / snapshot-diff) を隔離・将来は別ファイル比較。詳細は [`USER_REQUEST_LEDGER.md`](USER_REQUEST_LEDGER.md) の WP-005 行
