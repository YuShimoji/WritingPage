...
> 最終更新: 2026-07-13 / v0.3.32（current-main CI trust recovery closed）
...
Current-main CI trust recovery は `cf4b432` / run `29198025986` で閉鎖済み。次の最優先 quality milestone は release-readiness checkpoint とし、automated Web gates、UI capture evidence の所有と鮮度、Electron/package-only human gate を一つの bounded decision surface から読めるようにする。Documents tactile review は user-owned の deferred review debt として並行可能で、checkpoint を止めない。WP-004 parity pack は新しい preview / Reader 差分が出た時だけ起動する。
