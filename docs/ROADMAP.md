...
> 最終更新: 2026-05-15 / v0.3.32（Remote sync and restart roadmap handoff）
...
- ドキュメント権限: `CURRENT_STATE` / `USER_REQUEST_LEDGER` / `ROADMAP` / `FEATURE_REGISTRY` は active help cleanup 後の現行説明へ同期済み。2026-05-25 の Import Roundtrip Hardening と clean sync は `CURRENT_STATE` と `USER_REQUEST_LEDGER`、詳細 handoff は `docs/verification/2026-05-25/remote-sync-import-roundtrip-handoff.md` を正とする
...
Remote sync handoff after Import Roundtrip Hardening is recorded in `docs/verification/2026-05-25/remote-sync-import-roundtrip-handoff.md`. Restart on another terminal by pulling `main`, confirming `HEAD...origin/main = 0 0`, then reading `CURRENT_STATE` -> `INVARIANTS` -> `INTERACTION_NOTES`.
