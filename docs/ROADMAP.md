...
> 最終更新: 2026-06-25 / v0.3.34（WP-005 preview entry Slice A）
...
- **WP-005（session 94 起票）**: プレビュー・比較ツール再設計。スライスA は done: 分割ビュー比較の公開入口を structure sidebar / Electron menu から外し、command palette 比較コマンドを隠した。次はスライスB: MD プレビューをリッチプレビュー化 (タイピング/スクロール Controller アクティベート)。スライスC: 比較ツール (chapter-compare / snapshot-diff) を隔離・将来は別ファイル比較。詳細は [`USER_REQUEST_LEDGER.md`](USER_REQUEST_LEDGER.md) の WP-005 行
