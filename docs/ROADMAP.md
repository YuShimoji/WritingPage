...
- **WP-001**: ~~コマンドパレットのモード切替後フォーカス~~（session 46 済）。~~狭幅ツールバー折り返し・余白~~（session 48: CSS + geometry E2E 強化）。session 71 で次トピックを **「アシスト／メタ系ガジェットの発見性」** に選定（`USER_REQUEST_LEDGER` 参照）
