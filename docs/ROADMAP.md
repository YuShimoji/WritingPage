...
> 最終更新: 2026-07-25 / v0.3.32（G3 H2 internal-review entry）
...
- CI: implementation baseline `cf4b432 fix: recover current-main CI trust` / `CI E2E` run `29198025986` は `completed / success`。remote log は smoke pass、unit 16/16、full Playwright 594 passed / 4 skipped を直接記録し、job `e2e` と step `Run acceptance gates` も成功している
- コア機能: 95% 成熟
...
- ドキュメント権限: Current product baseline / main は `889a642`。G3 H2 observation ingestion とinternal-review入口はbranch `feat/g3-h2-compact-observation-ingestion` / `67b951b`、CI run `29771311517`、`docs/verification/2026-07-21/g3-h2-compact-observation-ingestion.md` を正とする。2026-07-25の再開状態と条件付きgoal stackは `CURRENT_STATE` / `docs/verification/2026-07-25/latest-remote-dev-ready-supervisor-goal-proposal.md`。Latest current-main CI trust proof は `docs/verification/2026-07-12/current-main-ci-trust-recovery.md`。Latest Design Cockpit / writing trust dashboard proof は `docs/PROJECT_COCKPIT.md` / `docs/verification/2026-07-06/design-cockpit-writing-trust.md`。Latest Editor Trust product proof は `project-import-recovery-continuation-proof` / `docs/verification/2026-06-25/project-import-recovery-continuation-proof.md`。
- 直近 done: Current-main CI trust recovery、Design Cockpit writing trust dashboard、One-click update launcher、Effect settings writer-facing wording、Command palette Markdown source dev gate、Rich text block align persistence、WP-005 Slice C comparison isolation、WP-005 Slice B MD preview rich-preview activation、WP-005 Slice A preview entry cleanup、Project import recovery continuation proof、Project import safe failure signal、Rich heading closure checklist、Rich heading placeholder/caret polish、WP-SAVELOAD-001 Editor Trust Vertical Slice、Rich editing typed heading shortcut、First-use Save Help、Chapter Creation Daily Flow、Export Trust Proof、Save / Resume Trust Audit、Docs authority hygiene after active help cleanup、Active help mode wording cleanup、Local Gadget Mod migration lane closeout、`PomodoroTimer` Local Mod migration、PomodoroTimer Mod feasibility audit、`HUDSettings` Local Mod migration、`MarkdownPreview` Local Mod migration、Local Gadget Mod MVP、A3 Floating memo command palette限定実験（`db3b3df` として push 済み）、A2 daily writing proof、Writing UX map + A1 Floating memo reframe、right window drag handle invisible-drag fix、left nav / unified shell narrow fixes、SP-081(エディタ体験再構築), SP-080(JSONプロジェクト保存)
- スコープ整理 (2026-03-23): EPUB/DOCX/画像管理/Canvas/Google Keep/プラグイン正式化/サイドバーP2-3/長期ビジョン7件を除外

### 2026-04-30 現行ロードマップ（並列 2 レーン）

詳細な次スライス表は [`docs/USER_REQUEST_LEDGER.md`](USER_REQUEST_LEDGER.md) を正とする。レーンは並列で管理するが、実装 PR / 作業単位は **1 user outcome** に絞る。完了に不可欠な最大3件の密結合した変更は同じ package に含めてよい。
...
2026-07-06 の Design Cockpit は、WP-SAVELOAD-001 以後の保存信頼を「レビュー時に状態を一画面で読める」方向へ狭く進めた slice。`Design Cockpit` command と `?designCockpit=1` は save state / document identity / editor surface / shell state / review summary を app-local dashboard に集約するが、storage schema、autosave、writing status chip、Documents、import/export、cloud/account/public sharing は変更しない。
...
Current-main CI trust recovery は `cf4b432` / run `29198025986` で閉鎖済み。Release-readiness checkpointとcompact observation ingestionはH2 branch `67b951b` まで実装・検証済みで、exact package derivativeは `READY_FOR_INTERNAL_RELEASE_REVIEW`。次の最優先quality milestoneはbounded internal reviewとH2 integration判断であり、外部release承認ではない。Documents tactile reviewはuser-ownedのdeferred review debtとして並行可能。WP-004 parity packは新しいpreview / Reader差分が出た時だけ起動する。

### 次スライス候補 (WP-004 / WP-001 / writing trust)

詳細は [`docs/USER_REQUEST_LEDGER.md`](USER_REQUEST_LEDGER.md) の表を正とする。実装時は **1 user outcome** に絞り、別の意思決定軸を混ぜない。
...
- **WP-001**: ~~コマンドパレットのモード切替後フォーカス~~（session 46 済）。~~狭幅ツールバー折り返し・余白~~（session 48: CSS + geometry E2E 強化）。~~アシスト／メタ系ガジェットの発見性~~（session 72 実施）。~~サイドバー「編集」カテゴリの情報密度~~（session 73: カテゴリ見出し）。~~ロードアウトプリセットとガジェット既定の整合~~（session 75 実施）。~~編集カテゴリの個別ガジェット説明（B1）~~（session 84 実施）。~~他カテゴリ（assist / advanced）のガジェット説明整理~~（session 86 実施）。~~assist/advanced のコマンドパレット導線（`gadget-assist` / `gadget-advanced` + E2E）~~（session 88 実施）。~~Focus パネル UI 摩擦 6 件（エッジホバー即応化 + overlay 化 + セクション折りたたみ廃止 + 見出しメッセージ撤去 + 下部 UI 撤去 + 「新しい章」ボタン配置）~~（session 91 実施、Electron 再ビルド完了）。**ステータス（session 91）**: 監視モードで 1 件復帰消化後、再び監視モードへ。**ユーザーが体感で新規の摩擦を特定した時のみ** 1 outcome に昇格してスライス化する
...
- **中期（別起票）**: ブロック段落の左・中・右揃え — P2 save/resume proof は `rich-text-block-align-persistence` で完了。以後は新しい整列バグや縦書き / RTL など別軸の明示要求が出た時だけ新規スライス化する
