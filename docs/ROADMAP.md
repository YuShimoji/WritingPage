...
> 最終更新: 2026-06-03 / v0.3.32（Remote sync and current-context handoff）
...
- ドキュメント権限: `CURRENT_STATE` / `USER_REQUEST_LEDGER` / `ROADMAP` / `FEATURE_REGISTRY` は active help cleanup 後の現行説明へ同期済み。2026-06-03 の cross-terminal restart 文脈は `CURRENT_STATE` と `USER_REQUEST_LEDGER`、詳細 handoff は `docs/verification/2026-06-03/remote-sync-current-context-handoff.md` を正とする。Import Roundtrip Hardening の product proof は `a56671b test: harden import roundtrip`
...
Import Roundtrip Hardening is now done. The writing trust lane has proof for local autosave/resume, TXT/JSON export contents, chapter creation daily flow, first-use comprehension of the save/takeout model, and safe JSON return from external takeout. The next work should not reopen chapter creation, first-use help, export trust, or import trust unless a new failure appears; choose from Rich Editing Heading Shortcut Decision or stale spec reconciliation.
...
Remote sync and current-context handoff is recorded in `docs/verification/2026-06-03/remote-sync-current-context-handoff.md`. Restart on another terminal by pulling `main`, confirming `HEAD...origin/main = 0 0`, then reading `CURRENT_STATE` -> `INVARIANTS` -> `INTERACTION_NOTES`.

次は、初回説明・章作成・JSON 書き出し・JSON 読み込みそのものではなく周辺の摩擦を別スライスで選ぶ。Rich editing の `# 見出し` 体験を変える場合は `Rich Editing Heading Shortcut Decision`、古い仕様表を正本へ寄せる場合は stale spec reconciliation として分ける。WP-004 parity pack は新しい preview / Reader 差分が出た時の user-actor release gate として扱う。
...
- **WP-005（session 94 起票）**: プレビュー・比較ツール再設計。スライスA: 分割ビュー edit-preview 廃止 + 導線整理。スライスB: MD プレビューをリッチプレビュー化 (タイピング/スクロール Controller アクティベート)。スライスC: 比較ツール (chapter-compare / snapshot-diff) を隔離・将来は別ファイル比較。詳細は [`USER_REQUEST_LEDGER.md`](USER_REQUEST_LEDGER.md) の WP-005 行
