...
| 手動 | タイポグラフィ体感、テーマの細部、複雑なマルチウィンドウ、オフライン前提 | `docs/MANUAL_TEST_GUIDE.md` または対象トピックの verification note |
...
- 長時間執筆の疲労・快適性
- WYSIWYG カスタム Undo の **主要境界**（Space / **Enter** 直後の 1 段 Undo、blur 後セグメントの 1 段 Undo）は [`e2e/wysiwyg-editor.spec.js`](e2e/wysiwyg-editor.spec.js) の FR-007 テストで回帰。**Undo 直後の Redo**、**連続 Undo に続く連続 Redo（Space 区切りの複数セグメント）**、**Space 境界のあと長い連続入力を 1 段 Undo** も同 spec で回帰。**IME 確定の native visual** は OS・ブラウザの IME 実装差が大きくヘッドレスで再現困難なため手動 remainder とし、app-owned な `compositionend` フラッシュ、repaint hook の非破壊性、Rich editing contenteditable の `spellcheck=false` surface contract は同 spec で回帰する。**極端な長文連打の体感**（パフォーマンス・細かい段数）は引き続き手動（[`docs/FEATURE_REGISTRY.md`](FEATURE_REGISTRY.md) FR-007）
