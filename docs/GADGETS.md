...
## HUDSettings Local Mod

HUD（Heads-Up Display）の表示設定を管理する Local Gadget Mod です。フェードイン/アウト型ミニHUDの位置・色・サイズ・メッセージなどを調整できます。

`HUDSettings` は built-in wrapper ではなく、`hud-settings-gadget` を設定モーダル `ローカルMod` で有効化し reload した時だけ advanced category に表示されます。HUD 本体、autosave HUD、command palette の HUD 表示は built-in のまま維持します。
...
1. 設定モーダル `ローカルMod` で `HUD設定` を有効化し、reload する
2. left nav の `advanced` category で「HUD設定」を開く
3. 各項目を調整
4. 「設定を保存」ボタンをクリックして反映
5. 即座にHUDの表示が更新されます
...
2. **HUD 拡張とカスタマイズ深化**
   - HUDSettings Local Mod へフォント/幅調整UIなどを段階的に追加済み。
3. **Wiki / ノードグラフ連携**
