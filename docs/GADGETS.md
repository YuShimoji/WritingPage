...
> 本節は現行コードに未反映の検討事項をまとめています。詳細な優先度・スケジュールは `docs/ROADMAP.md` を参照してください。

### ガジェット/パネル設計の将来タスク (将来案)

1. **ガジェット整理・統合** (`docs/ROADMAP.md` Priority B)
   - 28個の統廃合検討。類似ガジェット (Typography + FontDecoration, UISettings + UIDesign 等) の統合。
2. **Wiki / ノードグラフ連携** (`docs/ROADMAP.md` Priority C)
   - バックリンクUI統合、`[[wikilink]]` 構文の自動パース、グラフビューのUI統合。
   - 進める際は `docs/ARCHITECTURE.md` の「ガジェットベースアーキテクチャ」と整合を取る。
3. **Embed / SDK 連携の拡張**
   - Embed時のロードアウト反映や HUD の軽量化。`docs/EMBED_SDK.md` を併読。

### 参照ガイド

- ロードマップ: `docs/ROADMAP.md` — 機能強化の優先度と計画。
- アーキテクチャ: `docs/ARCHITECTURE.md` — Region/Panel/Gadget 概念モデル。
