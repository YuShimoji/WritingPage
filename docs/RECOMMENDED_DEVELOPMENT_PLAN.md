...
1. **WP-001**: ~~ロードアウトプリセットとガジェット既定の整合~~（session 75 完了）。~~session 77: モード用語のオペレーター／長命メモ整合~~。~~session 78: 歴史 spec 注釈 + Wiki コマンド検索語~~。~~session 79: **ガジェット常設／ドック／優先度の正文化**（`spec-writing-mode-unification-prep` + `loadouts-presets` コメント）~~。次は **台帳の摩擦 1 件** または初回折りたたみ等（要検討）。
