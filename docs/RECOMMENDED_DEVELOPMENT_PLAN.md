...
最終更新: 2026-04-15（session 94 — E2E 整理 + 手動テスト環境 + WP-005 起票）
...
1. **WP-005 スライスA**: 分割ビュー edit-preview モード廃止 + 導線整理。MD プレビューとの重複解消。
2. **WP-005 スライスB**: MD プレビューにタイピング/スクロール Controller アクティベートを追加し「リッチプレビュー」化。WYSIWYG 編集面の横で「読者が見る表示」を並べて確認可能に。
3. **WP-005 スライスC**: 比較ツール (chapter-compare / snapshot-diff) を隔離。導線をコマンドパレット + サイドバー「構造」に集約。将来は別ドキュメント比較も。

### 中期（3〜6 スライス）

- WP-005 完了後、`WP-004 Phase 3` を継続収束し、再生オーバーレイと編集経路の視覚・操作一貫性を固定。
...
- WP-001 は引き続き監視モード（体感トリガー発火時のみ 1 トピックに昇格）。
