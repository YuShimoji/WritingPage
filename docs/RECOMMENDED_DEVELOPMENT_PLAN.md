...
1. **WP-001**: ~~ロードアウトプリセットとガジェット既定の整合~~（session 75 完了）。~~session 77: モード用語のオペレーター／長命メモ整合~~。session 78: **歴史 spec（`spec-mode-architecture`）注釈** + **物語Wikiのコマンドパレット検索語拡張**。次は台帳の **摩擦削減トピックを 1 件ずつ**継続。
