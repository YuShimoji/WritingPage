...
- [ ] ツールバー左端の「メニュー」ボタンでサイドバーの開閉ができる
- [ ] ツールバー右側の「ツールバー」ボタンでツールバー表示/非表示を切り替えできる（状態は保持される）
- [ ] ツールバー非表示時、右下に丸い「ツールバーを表示」ボタン（キーボードアイコン）が現れ、それを押すとツールバーが再表示される
- [ ] ツールバー右側の「フルスクリーン」ボタンでフルスクリーン切替が動作する
- [ ] 右下の「ツール」ボタン（歯車アイコン）でクイックツールパネル（フローティング）が開閉できる
...
- [x] テキスト選択時にツールチップが表示される（Selection Tooltip v1 実装済み、EDITOR_EXTENSIONS.md 準拠）
- [x] 太字/斜体/取り消し線/リンク/画像/区切り/ルビが仕様通りに挿入される
