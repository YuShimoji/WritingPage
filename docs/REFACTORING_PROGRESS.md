...
| # | テスト項目 | 期待される動作 | 状態 |
|---|-----------|--------------|------|
| 1 | サイドバー開閉 | 「メニュー」ボタンで開閉できる | 調査中 |
| 2 | タブ切り替え（クリック） | 構造/タイポ/アシスト/Wiki が切り替わる | 動作確認済み |
| 3 | タブ切り替え（キーボード） | Alt+1-4で切り替わる | 要確認 |
| 4 | ガジェット追加 | 「+ ガジェット追加」から追加できる | 要確認 |
| 5 | ガジェット削除 | ガジェットヘッダーの削除ボタンでガジェットを削除できる | 未テスト |
| 6 | ガジェット移動 | ガジェットヘッダーの移動ボタンで順序変更できる | 未テスト |
...
1. キーボードショートカット（Alt+1-4）の動作を確認
2. すべてのテスト項目を実行
3. Markdownlintのwarningを修正
