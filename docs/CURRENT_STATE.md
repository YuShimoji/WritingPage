...
| 前スライス (参考) | session 112: **package safe launcher 固定 + package gate 一次切り分け** — (1) [scripts/open-built-app.js](../scripts/open-built-app.js) を Windows 既定で packaged exe 優先に変更し、`app:open:dist` / `app:open:package` を追加。[scripts/open-packaged-app.ps1](../scripts/open-packaged-app.ps1) を新設し、`NODE_OPTIONS` / `ELECTRON_RUN_AS_NODE` / Playwright 系環境変数をクリアしてから PowerShell `Start-Process` で起動する正本導線を実装。(2) [docs/APP_LAUNCH_GUIDE.md](APP_LAUNCH_GUIDE.md) と [docs/MANUAL_TEST_GUIDE.md](MANUAL_TEST_GUIDE.md) に package 正本起動手順を追記し、起動エラー 2 種（Playwright preload 混入 / Windows quoting 崩れ）を「アプリ本体不具合ではない」と明記。(3) package 実機ゲートの一次結果: `Sections empty state` は **PASS**、`sidebar close/reopen` は **PASS**、`Reader 右上 controls 非重なり` は **PASS**、`drag strip` は **HOLD**（合成ドラッグでは位置変化を証明できず、実機手操作で再確認が必要）、`preview empty state` は **HOLD**（package 画面上で直接トグル証明を残せていない）、`終了→再起動永続化` は **HOLD**、`Windows DPI / zoomFactor` は **HOLD**。**Web 検証**: `npx playwright test e2e/sections-nav.spec.js e2e/ui-mode-consistency.spec.js e2e/accessibility.spec.js --reporter=line` **35 passed**、`npm run lint:js:check` clean。 |
