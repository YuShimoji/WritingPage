...
### Wiki 機能の実装ステータス (SSOT)

Wiki機能（Story Wiki）は現在「Phase 1: 最小実装」段階です。以下は機能の制限事項と今後の予定です。

- **実装済み**:
  - ページの作成・編集・削除 (localStorage)
  - タグ管理
  - タイトル/本文/タグの検索
  - テンプレート（キャラクター/場所等）の選択挿入

- **未実装 / 制限事項**:
  - `[[Internal Link]]` 構文の解決とナビゲーション（リンク自体はテキストとして保存されるが、クリック遷移しない）
  - 画像ファイルの添付・貼り付け
  - AIによる自動生成・要約機能
  - ページ間関係のグラフ可視化
...
## 直近の開発項目 (v0.3.24 実装済み)

| カテゴリ | 項目 | 詳細 | ステータス |
|----------|------|------|------------|
| 復元機能 | Ctrl+Shift+Z | 最新スナップショットからの復元ショートカット追加 | Done |
| 復元機能 | 自動バックアップ | 復元前に現在の内容を自動でスナップショット保存 | Done |
| スナップショットUI | 最新強調表示 | アクセントカラーのボーダー＋「最新」バッジ | Done |
| スナップショットUI | プレビュー機能 | クリックでモーダル表示、ESCで閉じる | Done |
| ドキュメント | ショートカット一覧 | EDITOR_HELP.md にキーボードショートカット表を追加 | Done |

## 以前の開発項目 (v0.3.23 実装済み)
...
- **v0.3.24 (2025-11-30)**: 復元機能強化（Ctrl+Shift+Z ショートカット、プレビュー機能、最新強調表示）
- **次期リリース予定**: フローティングパネル（Phase E）/ ワードカウントヘッダー再配置（Phase D）
