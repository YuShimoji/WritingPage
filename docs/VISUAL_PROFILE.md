# VISUAL PROFILE — テーマ/背景/フォント/余白/本文表示

本書は、テーマ/背景色/フォント/余白/本文幅/作業シーンなどの**見た目に関する設定群**を「Visual Profile」としてまとめて扱うための仕様メモです。
...
- **ユーザー視点の整理**
  - 「テーマ」「背景色」「フォント」「余白」「作業シーン」がバラバラではなく、
    「執筆面の雰囲気セット（= Visual Profile）」として理解できるようにする。
- **内部実装の整理**
  - 現在は `ThemeManager` / Themes / Typography / EditorLayout / VisualProfile gadget が個別に設定を持っている。
...
- **作業シーン**
  - `Typography Pack` が本文サイズ、行間、字間、段落字下げ、見出し preset、ルビ可視性をまとめて適用。
- **統合シェル状態**
  - command palette / left nav / Reader surface / 再生オーバーレイ / window controls island の状態モデルは `docs/INTERACTION_NOTES.md` が正本。
  - Visual Profile は公開 UI 状態の第一級切替ではなく、主にテーマ・フォント・本文幅・作業シーンの一括適用を担う。
...
- **既存設定とのマッピング方針の整理**
  - `ZenWriterStorage` の settings と VisualProfile の対応（テーマ名/色/フォント/余白/本文幅/作業シーン）を明文化する。
- **適用フローの骨組みを定義**
...
- command palette / left nav / Reader surface など、統合シェル状態の公開操作を Visual Profile の主責務へ戻すこと。
...
type VisualProfileId = 'default' | 'minimal-dark' | 'plain-light' | string;
...
interface VisualProfile {
  ...
  // legacy/internal compatibility field。公開 UI 状態の第一級概念にはしない。
  uiMode?: 'normal' | 'focus';
}
```
...
- Typography Pack state ↔ `profile.typographyPack`
- `settings.ui.uiMode` ↔ `profile.uiMode`（互換入力のみ。新規 built-in / user-saved profile の主機能にしない）
...
function applyVisualProfile(profile: VisualProfile) {
  ...
  // 4) 作業シーン
  //   profile.typographyPack があれば Typography Pack を適用する。

  // 5) legacy/internal compatibility
  //   profile.uiMode が既存データから渡された場合だけ ZenWriterApp.setUIMode(profile.uiMode) へ委譲する。
}
```
...
- 新規の built-in profile とユーザー保存 profile は、既定では `uiMode` を保存しない。必要になった場合も互換目的の明示的な別スライスで扱う。
...
- **配置場所**
  - 左サイドバーの theme カテゴリ内、VisualProfile ガジェットとして独立。
    （旧 TypographyThemes から分割済み）
- **Phase A で実装する UI 要素**
  - プロファイル選択セレクトボックス（例: `標準レイアウト`, `集中（ダーク）`, `ブランク（ライト）`, `カスタム`）。
  - 選択に応じて、`theme` / `fontFamily` / `editorFontSize` / `uiFontSize` / `lineHeight` / `typographyPack` を一括適用します。
...
- **ユーザーから見える変化のイメージ**
  - セレクトボックスでプロファイルを切り替えると、
    ...
    - 本文幅
    - 作業シーン
    がまとまって変わり、「執筆面の見た目を切り替えた」感覚を得られます。
- **今後の拡張（Bフェーズ以降の候補）**
...
- 組み込み Visual Profile の案出し（2〜3個）と、それぞれのテーマ/フォント/余白/作業シーンの組み合わせを決める。
