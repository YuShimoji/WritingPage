...
interface VisualProfile {
  ...
  // 内部互換 UI state（公開 UI の第一級概念にはしない）
  uiMode?: 'normal' | 'focus';
}
```
...
- **Phase A で実装する UI 要素**
  - プロファイル選択セレクトボックス（例: `標準レイアウト`, `集中レイアウト（ダーク）`, `素朴なライト`, `カスタム`）。
  - 選択に応じて、`theme` / `fontFamily` / `editorFontSize` / `uiFontSize` / `lineHeight` を一括適用します。
