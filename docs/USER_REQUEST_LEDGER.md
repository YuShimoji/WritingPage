...
- **Command palette Markdown source dev gate**: Rich text block align persistence 後の fresh one-topic slice として完了。`editor-surface-markdown` は `devOnly` になり、通常配布相当では command palette 検索に出ない。開発者モードでは `Markdown ソース` escape hatch として残り、説明文も開発者モード境界を明示する。検証 anchor は `docs/verification/2026-06-25/command-palette-markdown-source-dev-gate.md`。
- **Effect settings writer-facing wording**: `effectBreakAtNewline` / `effectPersistDecorAcrossNewline` は接続済み設定のまま、command palette と UI Settings の visible wording だけを writer-facing にした。表示名は `改行後の装飾を切る` / `改行後も装飾を続ける`。storage keys、command ids、Enter 挙動、永続化契約は未変更。検証 anchor は `docs/verification/2026-06-26/effect-settings-writer-facing-wording-audit.md`。
- **One-click update launcher**: 通常確認用に `ZenWriter-UpdateAndLaunch.cmd` / `npm run app:update:open` / Start Menu `Zen Writer Update and Launch` を追加。更新は clean worktree の時だけ `git pull --ff-only`、その後 `npm run build` と既存 `app:open:dist`。既存 `Zen Writer.url` quick open、dev server、Electron packaged check は別導線として維持。検証 anchor は `docs/verification/2026-06-26/one-click-update-launcher.md`。
- **IME underline repaint fix**: Rich editing で日本語 IME 確定後に native composition underline が repaint まで残る観測に対し、`compositionend` 後に contenteditable の内容・selection・Undo を触らない paint-only repaint tick を追加した。synthetic composition の E2E は app-owned repaint hook と非破壊性を固定するための最小回帰テストであり、native OS IME の最終 visual は必要なら手動確認として扱う。検証 anchor は `docs/verification/2026-06-28/ime-underline-repaint-fix.md`。
...
- **MarkdownPreview Local Mod migration**: `MarkdownPreview` は built-in wrapper ではなく `markdown-preview-gadget` として manifest に登録する。preview pipeline / `ZenWriterEditor.togglePreview()` / command palette / Reader / developer-mode Markdown source は built-in のまま維持し、Mod は開閉ボタンと `preview.syncScroll` 設定だけを持つ。
...
| Done | WP-SAVELOAD-001 Editor Trust Vertical Slice | 保存・復帰・入出力を横断する `writing-trust-workflow-001`。明示保存、自動保存 reload、chapterMode 親 document 対象、TXT / Markdown / JSON export、JSON import roundtrip、不正 JSON 安全失敗、保存失敗表示を確認済み | assistant / writing trust |
| Done | Project import recovery continuation proof | `0c21466` の safe-failure message の次を完了。invalid JSON import 失敗後に現在の原稿へ追記でき、reload/resume 後も元本文と追記が維持されることを focused proof で確認済み | assistant / project recovery |
...
- Status: done. This is a docs-only handoff to keep restart context in project files before pushing to `origin/main`; product code and contracts are unchanged.
- Product proof anchor remains `a56671b test: harden import roundtrip`; pre-handoff context anchor was `b9948fb docs: hand off import roundtrip sync`.
- Project context anchor: `docs/verification/2026-06-03/remote-sync-current-context-handoff.md` records the clean-state check, restart route, trusted/non-reopen areas, and next entry points.
- Current restart order: `docs/CURRENT_STATE.md` -> `docs/INVARIANTS.md` -> `docs/INTERACTION_NOTES.md`; use `docs/USER_REQUEST_LEDGER.md` / `docs/ROADMAP.md` only when choosing the next slice.
- Next candidates stay separate: `Rich Editing Heading Shortcut Decision` first, `Docs Hygiene: stale spec reconciliation` second, and WP-004 parity pack only as a user-actor release gate when a new preview / Reader difference appears.

# 2026-06-04 Remote sync and cross-terminal handoff

- Status: done. Local `main` was fast-forwarded to `d007bf0 docs: hand off current sync context` after `origin/main` advanced from the previous local anchor.
- Product proof anchor remains `a56671b test: harden import roundtrip`; this handoff is docs-only and does not change product code, storage behavior, UI wording, dependencies, DB/auth/API behavior, or Electron/package behavior.
- Project context anchor: `docs/verification/2026-06-04/remote-sync-cross-terminal-handoff.md` records the pull, clean-state check, restart route, trusted/non-reopen areas, and next entry points.
- Current restart order: `docs/CURRENT_STATE.md` -> `docs/INVARIANTS.md` -> `docs/INTERACTION_NOTES.md`; use `docs/USER_REQUEST_LEDGER.md` / `docs/ROADMAP.md` only when choosing the next slice.
- Next candidates stay separate: `Rich Editing Heading Shortcut Decision` first, `Docs Hygiene: stale spec reconciliation` second, and WP-004 parity pack only as a user-actor release gate when a new preview / Reader difference appears.

# 2026-06-05 Issue #118 / PR #119 meta-review gate

- Status: corrected to non-blocking stale/reference context. Issue #118 / PR #119 must not continue as implementation work, but their GitHub close state does not block current-main product work.
- Current judgment: PR #119 is not an Issue #118 implementation PR. It carries SP-073 PathText freehand drawing, and that exact tree is already present on `main` as `49c3c2f feat: SP-073 Phase 4 フリーハンド描画...`.
- Branch risk: `feature/ISSUE-118-postmessage-security` is stale. Merging from it now would be a broad reverse diff against current `main`, including deletion of current docs, E2E, Local Gadget Mod, and writing-trust/import-trust artifacts.
- Issue #118 handling: current `main` already shows strict child-bridge parent-source/origin checks, `allowedOrigin` sends, targetOrigin SDK handling, cross-origin demo, and dev-check security patterns. Treat the issue as optional stale bookkeeping or a current-main missing-DoD audit only when embed security is explicitly selected.
- Project context anchor: `docs/verification/2026-06-05/issue-118-pr-119-meta-review.md`.
- Next product decision: do not wait on PR / Issue cleanup. Return to `Rich Editing Heading Shortcut Decision` or stale spec reconciliation unless the user explicitly prioritizes embed security; if embed security is selected, audit current `main` only and do not reuse PR #119.

# 2026-06-05 Remote sync after GitHub artifact authority correction

- Status: done. Local `main` was checked with `git fetch --prune origin` and `git pull --ff-only origin main`; remote was already up to date at `c272503 docs: downgrade stale github artifacts`.
- Product proof anchor remains `a56671b test: harden import roundtrip`; this handoff is docs-only and does not change product code, UI wording, storage behavior, dependencies, DB/auth/API behavior, or Electron/package behavior.
