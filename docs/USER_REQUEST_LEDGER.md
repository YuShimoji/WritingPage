...
- **WP-005 Preview / Comparison entry cleanup**: Slice A is done. Public split-view comparison entry points were removed from the structure sidebar and Electron menu, and command palette comparison commands were hidden. Current public model is MD preview for editor-adjacent rendering, Reader for read-only review overlay, and comparison deferred to future Slice C. Verification anchor: `docs/verification/2026-06-25/wp005-preview-entry-slice-a.md`.
...
| Next | WP-005 Slice B MD preview rich-preview activation | Slice A の次。MD preview をリッチプレビュー化し、typing / scroll controller の activate 条件を整理する。比較ツールは Slice C へ分ける | assistant / preview-comparison |
