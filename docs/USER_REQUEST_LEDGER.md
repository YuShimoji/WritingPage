...
- **Remote sync / cross-terminal handoff**: Latest editor product proof anchor is `1e33e38 feat: add rich editing heading shortcut`. Latest docs reconciliation proof is `4cb49ee docs: reconcile ledger handoff anchor`; import baseline remains `a56671b test: harden import roundtrip`. Latest context handoff is `docs/verification/2026-06-08/remote-sync-context-handoff-after-ledger-anchor.md`. Before this handoff, `git fetch --prune origin` completed, `main...origin/main` was clean, and `HEAD...origin/main = 0 0`. On another terminal, run `git pull --ff-only origin main`, confirm clean `main...origin/main` and `HEAD...origin/main = 0 0`, then read `docs/CURRENT_STATE.md` -> `docs/INVARIANTS.md` -> `docs/INTERACTION_NOTES.md`; read `docs/USER_REQUEST_LEDGER.md` / `docs/ROADMAP.md` only when choosing the next slice, and treat project docs rather than chat history as the handoff authority.
