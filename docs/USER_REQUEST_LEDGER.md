...
- **WP-005 Preview / Comparison entry cleanup**: Slice A and Slice B are done. Public split-view comparison entry points were removed from the structure sidebar and Electron menu, and command palette comparison commands were hidden. MD preview is now proven as a visible editor-adjacent rich-preview surface with typing / scroll controller activation and live content refresh, while Reader remains the read-only review overlay. Comparison remains deferred to future Slice C. Verification anchors: `docs/verification/2026-06-25/wp005-preview-entry-slice-a.md` and `docs/verification/2026-06-25/wp005-md-preview-rich-preview-activation.md`.
- **Remote sync / cross-terminal handoff**: Latest repo-context handoff before this product proof is `docs/verification/2026-06-24/remote-sync-after-project-import-safe-failure.md`; the latest Editor Trust product proof is now `project-import-recovery-continuation-proof`, with verification anchor `docs/verification/2026-06-25/project-import-recovery-continuation-proof.md`. The Rich heading review-dedup closure checklist remains `docs/verification/2026-06-22/rich-heading-feature-closure-checklist.md`; Rich heading functionality is accepted, placeholder/caret debt is resolved, and release-wide visual acceptance remains optional/non-blocking. Import baseline remains `a56671b test: harden import roundtrip`. On another terminal, run `git pull --ff-only origin main`, confirm clean `main...origin/main` and `HEAD...origin/main = 0 0`, then read `docs/CURRENT_STATE.md` -> `docs/INVARIANTS.md` -> `docs/INTERACTION_NOTES.md`; read `docs/USER_REQUEST_LEDGER.md` / `docs/ROADMAP.md` only when choosing the next slice, and treat project docs rather than chat history as the handoff authority.
...
| Next | WP-005 Slice C comparison isolation | chapter-compare / snapshot-diff を MD preview や Reader と混ぜず、将来の比較 surface として隔離する。別ファイル比較方向を扱う場合もこの slice で境界を切る | assistant / preview-comparison |
