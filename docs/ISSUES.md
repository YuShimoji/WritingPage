...
> - Issue #4 (インポート/エクスポート): SP-075 (Google Keep連携) が一部カバー。プラットフォーム別対応は別問題として維持
> - Issue #7 (SSOT): 新仕様7件 + spec-index.json 更新で部分的に解決方向。文書役割定義は引き続き必要

---

## 8) Editor モード UX 統合改善 (2026-03-19) — 未着手

チャプター管理の 3 点イシュー + ドキュメント操作の不整合。詳細は [docs/issues/2026-03-19_chapter-ux-issues.md](issues/2026-03-19_chapter-ux-issues.md) を参照。

- **Issue A**: 章モードが一方向移行でロールバック不可。chapterMode をデフォルト化すべき。
- **Issue B**: 文字数 (546 等) が Markdown ソース全体のバイト数で、DSL 構文を含む。プレーンテキストベースに変更が必要。
- **Issue C**: Legacy モードの「+ 新しい章」がドキュメント本文に見出しを直接挿入し、章タイトルと本文が混在する。
- **Issue D** (別 Worker 担当): 「構造 > ドキュメント」でファイルをクリックすると順序が入れ替わる / 内容が消失する。

提案 SP: SP-079 — Editor モード UX 統合改善
