...
- 手動確認依頼と次アクション選択を同じ質問に混ぜない。同一報告内で別節として併記することはできる
...
- 主観性の高い変更は完成品の後で好みを聞くのではなく、実装前に 2〜4 の方向と推奨を出す。明示した implementation decision gate での選択は、その outcome package 内の実装承認を兼ねる
- preference-driven な修正は指摘ごとの Prompt に分解せず 1 batch にまとめる。2 回収束しなければ、3 回目の微修正ではなく方向比較へ戻る
...
- **報告は自然文で書く** (2026-05-11 設定): 完了報告は「何を変えた / なぜ効くか / 残る不確実性 / 次の具体的な動き」を自然文で繋ぐ。`summary` / `evidence` / `risk` / `next owner` / `assistant next` / `差分の焦点` / `次の owner` のような **固定見出しや英語ラベルを出力構造として出さない**。これらは内部チェック項目であり、出力フィールドではない (ユーザーが明示的にその形式を要求した場合のみ可)。
- **`NEXT_WORK_SHORTHAND` 失敗モード**: 残作業レビューや次回作業導線が `P0/P1`、commit/test/path、または「こちら側で未処理なし」の短文に縮退し、各作業の目的・効果・必要条件・現在状態・次の動きが本文だけで分からない状態。ユーザーは何を選ぶと何が軽くなるか判断できず、次回の作業開始がまた status 確認から始まる。
  - 予防: `残作業` / `次回` / `優先順位` / `レビューしてください` が出たときは、作業ごとに目的・効果・必要条件・現在の状態・次の動きを自然文で説明する。**表は任意で固定列名は不要**。assistant が今すぐ動ける候補と user review 待ちを混ぜない。
- **完了報告の論理鎖**: 何が完了したか / 意図的に変えていないこと / workflow への効果 / 根拠 / 残る不確実性 / 次に動く主体 / user 返答後に assistant が閉じる作業 — これらの鎖が自然文で繋がっているか closeout 前に確認する。鎖が切れたまま「確認してください」「次をお願いします」で終えない。file path / line number は根拠として後ろに置くだけで、`meaning` の代替にしない。固定見出しを出力する要求ではない。
- **Prompt fragmentation の防止**: 監修 Prompt は micro-step の列ではなく 1 outcome package とする。開発側は package 内の関連修正・検証・CURRENT_STATE live block・Git follow-through を完了してから checkpoint を出し、軽微な中間報告で停止しない
...
| **設定 UI** | サイドバー **詳細設定** の **UI Settings** 内: `effectBreakAtNewline` はチェック **改行後の装飾を切る**（id: `effect-break-at-newline`）。`effectPersistDecorAcrossNewline` は **改行後も装飾を続ける**（id: `effect-persist-decor-across-newline`） |
