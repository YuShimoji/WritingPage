...
- [ ] `Escape` または外側操作で command palette が閉じ、通常執筆状態へシームレスに戻る
- [ ] Electron menu の `シェル` 系項目から command palette / left nav / Reader surface に到達できる
- [ ] 右上 window controls island 内の drag handle からウィンドウを移動できる
- [ ] 右上 island が非表示の状態では、同じ位置をドラッグしてもウィンドウが動かない
- [ ] 左上・本文・サイドバーに window drag region が残っていない
...
- left nav: root / 非表示（直前カテゴリは cue のみ）
- 編集面: リッチ編集表示
...
### 5.1 command palette / window controls

- [ ] 起動直後は visible top chrome / 上端 hover reveal / 常設 handle が表示されない
- [ ] F2 で command palette が表示され、フォーカスが shell 操作へ移る
- [ ] Escape / 外側クリックで command palette が閉じる
- [ ] 右上 window controls island は hover / focus 時だけ表示される
- [ ] 非表示時の island / handle は不可視 drag region として反応しない
- [ ] island 内 drag handle が Electron frameless window の移動導線として使える
...
- [ ] 「編集に戻る」ボタンで閉じる
- [ ] 閉じた後、command palette / left nav の状態が不自然に変化しない
- [ ] 縦書き/横書き切替ボタンが機能する
...
- [ ] 切替後にエディタ・left nav・command palette の色が変わる
...
### 編集
- [ ] MarkdownPreview: 標準 preset には出ず、developer/audit 用 custom loadout で明示した場合だけ MD プレビュー表示
- [ ] FontDecoration: 標準 preset には出ず、装飾は WYSIWYG toolbar / command palette 側を主導線にする
- [ ] TextAnimation: 小説・脚本標準 preset には出ず、VN preset では演出ツールとして表示
- [ ] Images: 画像挿入
- [ ] ChoiceTools: 選択肢・分岐
...
### 詳細設定
- [ ] UISettings: 日常 UI 設定（表示・文字サイズ・placeholder・自動保存）
- [ ] EditorAdvancedSettings: 高度な編集設定（リッチ編集改行・Textbox・浮遊パネル・ガジェット表示）
...
- [ ] 「検索」 > 検索パネルが開く
- [ ] 「保存」 > 保存実行
- [ ] `F2` / legacy toolbar 互換経路 > command palette が表示される
