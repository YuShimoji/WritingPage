...
本プロジェクトで把握している既知の問題をまとめます。修正優先度は `docs/ROADMAP.md` と合わせて検討してください。

- 現行バージョン: **0.3.29**
- 関連: `docs/PROJECT_HEALTH.md` / `docs/ROADMAP.md`

## 機能/表示

- [改善済み v0.3.19 → v0.3.28 で解消] ダークテーマで一部の境界色が弱く、コントラストが不足する可能性
  - Obsidian/Quiver風にリファイン済み（サイドバー暗め、エディタ一段明るめ）
...
- [解消済み v0.3.2] ツールバーを非表示にすると再表示のUIがなく、戻しづらい（リロードで復帰）
  - いまは画面右下の「ツールバーを表示」ボタン（キーボードアイコン）で再表示できます。キーボードショートカット Alt+W でも切り替え可能です。
...
- 監査ログ（JSON）の保存先・運用手順の整備
- Embed SDK の same-origin 判定と origin 検証の正規化
- プラグインシステムの「UI有無」の明確化
