...
| `{kenten\|text}` | 傍点 | GUI済み (WYSIWYGツールバー: 選択→適用/クリック→解除) |
...
手動介入: 演出系 DSL はツールバーGUIから挿入可能 (Q1解決)。属性設定モーダルで詳細指定。DSL直書きも可
...
| 装飾/表現 | DSLパース、Markdown変換、ルビ変換、Wiki検出、DSL挿入GUI (Q1解決)、傍点GUI (session 29) | 装飾選択 | — |
...
**現状**: session 29 で解決済み。WYSIWYGツールバーに傍点ボタン追加。選択→適用/既存クリック→解除。
**効果**: 傍点付与がGUIで完結。DSL記法 `{kenten|text}` とのラウンドトリップも維持。
