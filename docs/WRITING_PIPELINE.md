...
- ルビ/傍点の手動付与
- パステキスト(SP-073)のカーブ定義 -- Phase 2 UI未実装
- タイポグラフィパックの選択
...
| 記法 | 用途 | 自動化の余地 |
|------|------|-------------|
| `:::zw-textbox{type:"dialogue"}` | テキストボックス | WYSIWYGから挿入UIがあればGUI化可能 |
| `:::zw-typing{speed:30}` | タイピング演出 | 同上 |
| `{漢字\|かな}` | ルビ | WYSIWYG選択→ルビ付与UIで半自動化可能 |
| `{kenten\|text}` | 傍点 | 同上 |
...
### 状態
- **完成度: 85%**
- SP-073 Path Text Phase 2 UI未実装 (40%)
- DSLの手動記述が多い -- GUI挿入UIの拡充余地あり
- テキストボックス/タイピング演出のWYSIWYG挿入UIは未整備
...
以下はデザイナー体験に直結する設計判断であり、未決定。

### Q1: 演出系 DSL の挿入 UI をどこまで作るか

- **A**: 全 DSL 型にツールバー挿入ボタンを追加 (typing/dialog/scroll/pathtext)
- **B**: よく使う型のみ UI 化し、残りは手書きを維持
- **C**: コマンドパレットにスニペット挿入を追加 (ツールバー UI 追加なし)

### Q2: ルビ挿入 UI の形態

- **A**: テキスト選択 → ツールバーボタン → ルビ入力モーダル
- **B**: インライン入力 (選択テキストの上にルビ入力フィールドが出現)
- **C**: Markdown 記法のまま維持 (学習コストで対応)

### Q3: WYSIWYG での演出プレビュー範囲

- **A**: WYSIWYG でもタイピング/ダイアログを静的プレビュー表示
- **B**: Reader Preview を演出確認の専用段階として維持 (現状)
- **C**: WYSIWYG にリアルタイムアニメーションプレビューを実装
...
| 装飾/表現 | DSLパース、Markdown変換、ルビ変換、Wiki検出 | DSL記述、装飾選択、ルビ付与 | **DSL挿入GUI** (最大の改善余地) |
| プレビュー | 全装飾レンダリング、目次/ナビ生成、ドラフト除外 | モード切替、目視確認 | 差分プレビュー |
| 出力 | CSS埋め込み、リンク正規化、ナビ除去 | 形式選択、保存先指定 | EPUB/DOCX、一括出力 |
...
**現状**: テキストボックス (`:::zw-textbox`) やタイピング演出 (`:::zw-typing`) はDSL記法を手で書く必要がある。
**改善案**: WYSIWYGモードにブロック挿入UI (コマンドパレット or `/` スラッシュコマンド) を追加し、GUIからDSLブロックを生成する。
**効果**: 装飾工程の手動記述量を大幅に削減。DSL記法を知らなくても使える。
**関連**: SP-074のDSLパーサーは既に堅牢。入力側のUI不足が摩擦の本質。

### 2. ルビ/傍点の選択付与UI

**現状**: `{漢字|かな}` `{kenten|text}` を手動記述。
**改善案**: テキスト選択 → 装飾バーに「ルビ」「傍点」ボタン追加。選択テキストをDSL記法でラップ。
**効果**: 日本語小説で頻出するルビ付与の手間を半減。

### 3. EPUB出力 (出力工程の最大ギャップ)

**現状**: HTML/TXT/MDは出力可能だが、EPUB (電子書籍標準フォーマット) は未実装。
**改善案**: Reader Previewの装飾済みHTMLをepub-genで変換。チャプター構造→EPUB目次に直接マッピング可能。
**効果**: 「書いたものを配布する」パイプラインが完結する。
**判断待ち**: ユーザーが現状のブラウザ印刷PDFで足りているとの判断 (DECISION: 2026-03-08)。再評価が必要。
...
| 出力可能な形式数 | 4 (TXT/MD/HTML/PDF) | 5+ (EPUB追加) |
| データ消失リスク | 極低 (IDB+LS+スナップショット) | 維持 |
| DSL記法なしで使える装飾 | 基本装飾のみ (太字/斜体等) | テキストボックス/タイピングもGUI化 |
