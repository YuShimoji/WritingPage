...
|----|--------|------|-------------|----------|--------|------------|
| FR-001 | UI モード | Normal / Focus の切替と状態。Reader はモードではなく再生オーバーレイ導線（`data-reader-overlay-open`）として扱う | `docs/specs/spec-mode-architecture.md` | `js/app.js` (`setUIMode`), `js/reader-preview.js` | `ui-mode-consistency.spec.js`, `reader-preview.spec.js` | 2026-04-08 |
