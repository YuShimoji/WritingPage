...
- [x] ガジェット設定のインポート/エクスポートUI - GadgetPrefs ガジェットに実装
...
- [x] テーマプリセット拡張のための集中管理機構（ThemeRegistry 導入により集中管理基盤を実装。詳細は THEMES.md の C-2 セクションを参照）【カテゴリ: テーマ/配色】
- [x] UI配色と執筆エリア配色の分離（C-3 Step1/Step2/Step3 の基盤実装に続き、C-4 で UI と本文の配色分離を完了。テーマガジェットのカスタム色は本文中心で適用）【カテゴリ: テーマ/配色/エディタ設計】
- [x] ビジュアルUIエディタ - クリックで要素選択、個別またはタイプ別の一括色変更【フェーズ候補: v0.4.x / カテゴリ: UIエディタ・テーマ調整】 - TASK_033で完了
- [x] フローティングパネル機能 - サイドバーから切り離し、Docking/split モデル＋位置・サイズ・開閉状態の永続化、透明度調整・折りたたみUIまで実装済み（Phase E-1/E-2 および B-1）。柔軟なタブ配置や任意ガジェット切り離しは E-3/E-4 で対応【フェーズ候補: v0.4.x / カテゴリ: パネル/レイアウト】
- [x] 柔軟なタブ配置システム - 上下左右への配置、サイドバー内での順序変更【フェーズ候補: v0.4.x / カテゴリ: パネル/レイアウト】 - TASK_029で完了（2026-01-12）
- [x] タブへのガジェット動的割り当て完全実装 - ドラッグ&ドロップでガジェットをタブに追加【フェーズ候補: v0.4.x / カテゴリ: パネル/レイアウト】 - TASK_030で完了（2026-01-12）
...
- [x] スペルチェック（基本的なスペル提案）【フェーズ候補: v0.4.x 以降 / カテゴリ: 入力支援・言語処理】 - TASK_034で完了
- [x] リッチテキストエディタ（WYSIWYG） - contenteditableベースのエディタ実装、Markdownとの双方向変換【フェーズ候補: v0.5.x / カテゴリ: エディタ・リッチテキスト】 - TASK_031で完了
- [x] 画像位置調整・サイズ変更 - ドラッグ操作による画像の位置・サイズ変更機能【フェーズ候補: v0.4.x / カテゴリ: 画像・レイアウト】
- [x] コラージュレイアウト - 複数画像の配置・レイアウト機能【フェーズ候補: v0.4.x / カテゴリ: 画像・レイアウト】
- [x] テキストアニメーション - タイピング、フェードイン/アウト等のアニメーション効果【フェーズ候補: v0.4.x / カテゴリ: テキスト装飾・アニメーション】
- [x] フォント装飾システム - [bold], [italic], [underline] 等のMarkdown拡張構文とプレビュー【フェーズ候補: v0.4.x / カテゴリ: テキスト装飾・アニメーション】
- [x] コマンドパレット - ショートカットと合わせて操作の可視化（検索/置換/ガジェット操作を横断）【フェーズ候補: v0.4.x / カテゴリ: UI・操作支援】
- [x] 分割ビュー - 編集/プレビュー、章間比較、スナップショット差分表示【フェーズ候補: v0.4.x / カテゴリ: UI・表示】
- [x] フォーカスモード - 現在行以外を減光/ぼかし（集中支援、タイプライターモードと併用）【フェーズ候補: v0.4.x / カテゴリ: UI・集中支援】
...
- [x] Selection Tooltip v1 実装（テキスト選択に連動した装飾/挿入ツールチップ、EDITOR_EXTENSIONS.md 準拠）- 2025-12-05完了
...
- [x] Typora風ツリーペイン（ドキュメント/メモ/Markdown管理を統合するツリーUIの設計と段階的実装）【フェーズ候補: 長期 / カテゴリ: ドキュメントナビゲーション】
- [x] Wikilinks/バックリンク/グラフ - `[[link]]` 構文や `doc://` の可視化・相互参照グラフ【フェーズ候補: 長期 / カテゴリ: ドキュメントナビゲーション・リンク】
- [x] タグ/スマートフォルダ - ツリーペインにタグ軸、保存された検索、仮想フォルダ【フェーズ候補: 長期 / カテゴリ: ドキュメント管理】
- [x] Pomodoro/集中タイマー - HUD連携のセッション管理【フェーズ候補: 長期 / カテゴリ: 執筆支援・時間管理】
- [x] キーバインド編集 - ショートカットの再割当機能【フェーズ候補: 長期 / カテゴリ: UI・カスタマイズ】

## 優先度: 低

- [x] PRテンプレートの追加 - .github/pull_request_template.md に実装済み
- [x] OpenSpec strict 検証（specデルタのMUST/Scenario整備）と smoke 自己完結化（dev-checkの自動起動） - 2025-12-14完了
- [ ] 監査ログ（JSON）の保存先・運用手順を整備（生成/保持/PR・Issue への関連付け） - 現状は保存先が未整備
- [x] コード規約の明文化（ESLint/Prettier導入検討）【フェーズ候補: 継続 / カテゴリ: コーディング規約・静的解析】 - TASK_038で完了（2026-01-18）
...
- [ ] 監査: Embed SDK の same-origin 判定と origin 検証の正規化
- [x] 監査: `docs/GADGETS.md` の「現行実装」と「将来案/旧メモ」混在の解消 - TASK_040で完了
- [x] 監査: smoke/dev-check の期待値（未実装扱い）と現行実装の整合 - TASK_049で完了
- [ ] 監査: プラグインシステムの「UI有無」の明確化
...
- [x] 新規ドキュメント作成フローでの未保存確認・スナップショット退避の二重実行を整理（1回のダイアログ＋退避に統合） - app.js のフローで統一
...
#### E-1: フローティングパネル PoC（完了）

- [x] structure限定でフローティングパネル生成 - 2025-12-05完了
- [x] UISettingsから起動可能、ドラッグ移動・ドッキング切替対応

#### E-2: フローティングパネル本実装（完了）

- [x] 状態保存（位置・サイズ・開閉状態）を settings に永続化 - 2025-12-05完了
- [x] 初期表示位置の最適化（画面中央寄せ） - 2025-12-05完了
- [x] 全タブ対応（structure / typography / assist / wiki） - 2025-12-05完了
- [x] パネルタイトルのユーザー編集とタイトル永続化（ダブルクリックで変更） - 2025-12-05完了
- [x] 透明度調整UI - 2025-12-11完了（スライダーで20%〜100%調整、状態永続化）
- [x] パネル内容の折りたたみ/展開UI - 2025-12-11完了（ヘッダーのボタンで切り替え、状態永続化）
- [ ] ショートカットキーでのトグル（任意・次期対応）
...
- v0.3.24 でバックログ整理完了。
- v0.3.25 (REDESIGN) でサイドバー設計の見直しを完了。タブ削減、ドキュメント・エクスプローラー、およびツールバーのクリーンアップを実施。
