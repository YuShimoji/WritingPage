...
- [~] 用語整備・仕様遵守（UIラベルの統一）- 進行中：ロードアウト/プロファイル関連ラベル追加、残りのガジェットも順次対応
...
### フェーズ C: サイドバー構造安定化
- サイドバー＆ガジェット構造の安定化（データ属性ベースの安定セレクタ導入）
- SidebarManager と ZWGadgets の責務分離整理
- ガジェットグループの正規化

### フェーズ D: HUD/進捗機能拡張
- フェードイン/アウト型ミニHUD（位置・フェード時間・背景色のカスタマイズ）
