...
- [ ] リッチテキストエディタ（WYSIWYG） - contenteditableベースのエディタ実装、Markdownとの双方向変換【フェーズ候補: v0.5.x / カテゴリ: エディタ・リッチテキスト】
- [ ] 画像位置調整・サイズ変更 - ドラッグ操作による画像の位置・サイズ変更機能【フェーズ候補: v0.4.x / カテゴリ: 画像・レイアウト】
- [ ] コラージュレイアウト - 複数画像の配置・レイアウト機能【フェーズ候補: v0.4.x / カテゴリ: 画像・レイアウト】
- [ ] テキストアニメーション - タイピング、フェードイン/アウト等のアニメーション効果【フェーズ候補: v0.4.x / カテゴリ: テキスト装飾・アニメーション】
- [ ] フォント装飾システム - [bold], [italic], [underline] 等のMarkdown拡張構文とプレビュー【フェーズ候補: v0.4.x / カテゴリ: テキスト装飾・アニメーション】
- [ ] コマンドパレット - ショートカットと合わせて操作の可視化（検索/置換/ガジェット操作を横断）【フェーズ候補: v0.4.x / カテゴリ: UI・操作支援】
- [ ] 分割ビュー - 編集/プレビュー、章間比較、スナップショット差分表示【フェーズ候補: v0.4.x / カテゴリ: UI・表示】
- [ ] フォーカスモード - 現在行以外を減光/ぼかし（集中支援、タイプライターモードと併用）【フェーズ候補: v0.4.x / カテゴリ: UI・集中支援】
...
- [ ] Wikilinks/バックリンク/グラフ - `[[link]]` 構文や `doc://` の可視化・相互参照グラフ【フェーズ候補: 長期 / カテゴリ: ドキュメントナビゲーション・リンク】
- [ ] タグ/スマートフォルダ - ツリーペインにタグ軸、保存された検索、仮想フォルダ【フェーズ候補: 長期 / カテゴリ: ドキュメント管理】
- [ ] Pomodoro/集中タイマー - HUD連携のセッション管理【フェーズ候補: 長期 / カテゴリ: 執筆支援・時間管理】
- [ ] キーバインド編集 - ショートカットの再割当機能【フェーズ候補: 長期 / カテゴリ: UI・カスタマイズ】
