...
- [ ] live preview差分適用（リアルタイム性向上）【フェーズ候補: 長期 / カテゴリ: Markdownプレビュー性能（長文時パフォーマンス）, 詳細は「長期課題」の同名項目と統合】
...
- [ ] ワードカウント/進捗ヘッダーの再配置（エディタ上部から HUD/ガジェット等への退避と改行削減）【フェーズ候補: v0.4.x / カテゴリ: HUD・情報レイアウト】
- [ ] テーマごとのボタン/リンクアクセントカラー一括制御（テーマ/Visual Profile と連携したスタイル適用）【フェーズ候補: v0.4.x / カテゴリ: テーマ/配色】
...
- [ ] ワードカウント/進捗ヘッダーの再配置（エディタ上部から HUD への退避）【対応: 上記「優先度: 中」の同名タスクと統合】
- [ ] 執筆目標の進捗可視化強化【フェーズ候補: v0.4.x / カテゴリ: HUD・目標管理】
...
### 長期課題
- live preview差分適用（パフォーマンス）【対応: 上記「優先度: 高」の同名タスクと統合】
