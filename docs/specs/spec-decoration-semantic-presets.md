...
- semantic preset 5種 + explicit experimental tilt preset 1種 + 既存 legacy preset 3種
...
| `monologue` | 独白 | monologue | italic | fadein | soft | 0 | 0.98 | 標準の内面描写。傾きは含めない |
| `tilted-monologue` | Tilted monologue | monologue | italic | fadein | soft | -2 | 0.98 | 傾きが必要な内面描写の明示的・実験的 preset |
...
- `.zw-textbox--monologue`: 透過 + イタリック + 控えめな枠
- `.zw-textbox--tilted-monologue`: `.zw-textbox--monologue` と同等の静的表現 + 明示的な傾き
...
| Phase | 内容 | 状態 |
|------|------|------|
| P1 | semantic preset 5種定義 + explicit tilt preset 1種 + CSS + TextboxPresetRegistry | done |
| P2 | WYSIWYG/Markdown round-trip テスト (E2E 7件) | done |
| P3 | 統合UI: TB ドロップダウン (9プリセット一覧 + role バッジ) | done |
...
6. [x] Phase 3: TB ドロップダウンで9プリセットを統一アクセス (旧4ボタン → 1ドロップダウン)
...
- 新: 「TB ▼」ボタン1つ、クリックで全9プリセットのドロップダウン表示
