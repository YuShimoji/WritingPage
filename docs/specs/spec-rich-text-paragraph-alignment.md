...
- **記録のみ**（v0.3.x）。実装は **リッチテキスト・プログラム**（`[spec-richtext-enhancement.md](spec-richtext-enhancement.md)` と本書）のスライスとして着手する。
...
| 順 | スライス | 成果物の目安 |
| -- | -------- | ------------ |
| 1 | **永続化モデル** | ブロック単位の `text-align`（または論理値）をドキュメント JSON / フラグメントに載せる方針確定。Turndown / markdown-it との往復ルールを本書に追記 |
| 2 | **WYSIWYG コマンド** | `RichTextCommandAdapter` 経由でカーソルブロックに属性付与。UI はツールバーまたはコンテキスト（[`spec-richtext-enhancement.md`](spec-richtext-enhancement.md) canonical paths） |
| 3 | **プレビュー / Reader** | 生成 HTML にクラスまたはインライン `style`（方針に従う）を反映。WP-004 パイプラインとは役割分担を INTERACTION_NOTES に一言 |
| 4 | **回帰 E2E** | 左・中・右の最小シナリオを `wysiwyg-editor` またはプレビュー用 spec に追加 |
