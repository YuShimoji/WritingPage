...
| 4 | **回帰 E2E / 保存復帰** | **実装済み（2026-06-25）**: `rich-text-block-align.spec.js` が WYSIWYG「その他」メニューから段落揃えを実行し、Markdown 正本への保存、MD プレビュー、Reader、reload 後の Rich editing 復帰まで `data-zw-align` が残ることを確認する。`reader-wysiwyg-distinction.spec.js` は引き続きパイプライン残存 + `getComputedStyle(text-align)` の最小 assert を担う |
