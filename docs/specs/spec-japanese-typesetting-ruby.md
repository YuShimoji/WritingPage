...
- **記法**: `{kenten|文字列}` — ルビ記法と同形式。`kenten` キーワードで識別
- **CSS**: `text-emphasis: filled sesame` + `text-emphasis-position: over right`
- **プレビュー変換**: `editor-preview.js` でルビ変換の前に `{kenten|...}` → `<span class="kenten">` に変換
- **WYSIWYG変換**: `markdownToHtml()` 内で同じ変換を実行
- **逆変換 (Turndown)**: `<span class="kenten">` → `{kenten|...}` ルール追加
- **ガジェット**: FontDecorationガジェットに「傍点」ボタン追加。WYSIWYG時は `wrapSelectionWithSpan('kenten')`、テキストエリア時は `{kenten|` + `}` を挿入
- **印刷/PDF**: CSS `text-emphasis` はブラウザ印刷で保持される。未対応ブラウザではスタイルが無視されるが本文は可読

### 変更ファイル

- `css/style.css`: `.kenten` ルール追加
- `js/editor-preview.js`: `{kenten|...}` パーサー追加（ルビ変換の前に実行）
- `js/editor-wysiwyg.js`: Turndownルール `kenten` 追加 + `markdownToHtml()` 内変換追加
- `js/gadgets-editor-extras.js`: FontDecorationガジェットに傍点ボタン追加
...
5. 傍点・圏点が未対応環境でも可読なフォールバックになる。 -- done
