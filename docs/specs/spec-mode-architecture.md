...
執筆集中のための簡素化された状態。メイン横帯ツールバーは廃止済み。サイドバーは非表示になり、エッジホバーで必要 UI に一時到達する (BL-004)。

| 要素 | 表示 |
|------|------|
| サイドバー | 非表示。左エッジホバーで **チャプターリストパネル** がスライドイン |
| ガジェット | 非表示 |
| メインハブ | 既定は非表示。上端エッジ帯に **280ms 滞在**後にクイックツールとして表示（離脱後 **500ms** で収納）。上端中央の **極小ハンドル** からクリックでも開ける |
...
> **用語注意**: ここでの Reader は **アプリ内の読了レイアウト**（`data-ui-mode="reader"`）を指す。**スクリーンリーダー（支援技術）向けモードやボイス読み上げ専用設計ではない**。
...
| `js/edge-hover.js` | エッジ検出 (24px ゾーン)、280ms dwell / 500ms dismiss、グロー近接、上端ハンドル、UI ホバーガード |
...
- Focus モードで、画面にエディタのみ表示され、上端ホバー（短い滞在後）または上端ハンドルでメインハブに、左端ホバーで章パネルにアクセスできる
