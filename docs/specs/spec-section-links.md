...
### Phase 5: ゲームブック分岐UI (設計済み)

#### 設計思想

分岐リンクのスタイルは、テキストを基準とした段階的な表現強度で提供する。
既存のテキスト表現アーキテクチャ (SP-062) のエフェクト/プリセットシステムを
リンク表示にも転用可能にし、統一された演出言語で分岐を表現する。
...
- CSS: `.chapter-link--emphasis` (SP-062 の ornament / animation クラスを併用)
- DSL: `[text](chapter://target){style:"emphasis"}` で指定
- ホバー: テキストエフェクトの強度変化 (opacity / color shift)
...
- CSS: `.chapter-link--card` (SP-016 の zw-textbox ベース + hover transform)
- DSL: `[text](chapter://target){style:"card"}` で指定
- ホバー: scale + shadow 変化 (SP-062 の animation 転用)

#### エフェクト転用の仕組み

リンクスタイルは SP-062 のレイヤーモデルに接続する:

- **TextEffect**: リンクテキストの装飾 (bold / italic / outline)
- **Animation**: ホバー時 / 非ホバー時のアニメーション (fadein / shake)
- **Ornament**: リンク枠の装飾 (soft / burst / mono)

具体的には:
- `.chapter-link[data-link-effect]` 属性でエフェクトIDを指定
- TextExpressionPresetResolver が link 要素にも適用可能
- reduced-motion 時は animation を drop し、静的装飾のみ残す

```markdown
<!-- Layer 0 (デフォルト): テキストリンク -->
[森に入る](chapter://forest)

<!-- Layer 1: 強調テキスト + エフェクト -->
[森に入る](chapter://forest){style:"emphasis", effect:"fadein"}

<!-- Layer 2: カード + プリセット -->
[森に入る](chapter://forest){style:"card", preset:"inner-voice"}
```

#### 自動グループ化

連続する `chapter://` リンクは自動的に「選択肢グループ」として検出し、
`.chapter-choices` ラッパーで囲む。これにより:

- グループ全体のレイアウト (縦並び / 横並び) を CSS で制御
- グループの前後に区切り線を自動挿入
...
- [x] ゲームブック的分岐UIデザイン → Phase 5 設計: 3階層 (テキスト/強調/カード) + SP-062エフェクト転用
