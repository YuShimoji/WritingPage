...
- `doc://` は既知のパースバグ (`#section` 分離不可) があり、別途修正が必要 (既存 Issue #1)
...
- [ ] `chapter://` のID体系（UUID / slug / ユーザー定義ID）
...
- `js/modules/editor-wysiwyg.js`: `chapter://` リンクの認識と遷移処理
- `js/chapter-model.js`: リンク先の解決・順序管理
- `css/style.css`: ナビバーとリンクスタイル
- `index.html`: ナビバーのマウントポイント
- エクスポート処理: `chapter://` → `#anchor` 変換
