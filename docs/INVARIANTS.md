...
- writing status chip は Reader 非表示 / Floating memo lab 非表示の通常執筆時だけ出る非操作型 status とする。文字数、編集中/保存済み/保存失敗、最後に保存済みへ遷移した時刻だけを表示し、`pointer-events: none` を維持する。Reader / Floating memo lab 表示中は隠す。
...
- ガジェットは固定ラックではなく Local Gadget Mod として後から着脱できる境界を持つ。低頻度・実験的・個人用途の gadget は built-in へ直接追加せず、まず `js/plugins/<mod-id>/index.js` + `js/plugins/manifest.json` + 設定モーダル `ローカルMod` の enable 境界で扱う。Mod enable 状態は plugin manager、配置は loadout、gadget 内部設定は `ZWGadgets` prefs が担当する。
