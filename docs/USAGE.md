...
## 開発中の機能: Visual Profile（見た目のプリセット）

- テーマ/背景色/フォント/余白/表示モードといった「見た目」の設定を、今後は「Visual Profile」としてまとめて扱う予定です。
- 現時点では従来どおり個別の設定項目として操作しますが、将来的には「執筆モードのプリセット」を切り替える感覚で一括変更できるUIを追加する計画です。
- 開発中の仕様メモは `docs/VISUAL_PROFILE.md` を参照してください。
...
- ヘルプカテゴリ:
  - **はじめに**: 基本操作とショートカット
  - **エディタ機能**: 編集、検索、フォント装飾
  - **ドキュメント管理**: 複数ドキュメント、ファイル操作、バックアップ
  - **物語Wiki**: 設定管理、AI自動生成
  - **ガジェット設定**: ロードアウト、タブ管理
  - **表示モード**: 通常/フォーカス/ブランク
  - **テーマ・外観**: テーマ切替、カスタムカラー
  - **キーボード操作**: ショートカット一覧
