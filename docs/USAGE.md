...
Zen Writer は小説や物語の執筆に特化した Web エディタです。サーバー不要、ブラウザだけで動作し、LocalStorage に自動保存されます。推奨ブラウザ: Chrome / Edge / Firefox / Safari の最新版。
...
- `Ctrl + Shift + F` で全文検索パネルを開く（全ドキュメント横断、正規表現対応）
- 検索結果をクリックすると該当ドキュメントを開き該当箇所をハイライト
- `ESC` でパネルを閉じる

## 既知の制約

- ブラウザ/端末間の同期はありません（LocalStorage はブラウザごとに管理）
- 長大なテキストはブラウザのメモリ/LocalStorage制限（約5MB）の影響を受けます
- 重要な原稿は定期的に `.txt`/`.md` へエクスポートしてバックアップを推奨

## トラブルシュート

- 自動保存が効かない: プライベートブラウズやストレージ制限を確認してください
- ダウンロードされない: ブラウザのポップアップ/ダウンロード設定をご確認ください
- テーマが適用されない: ブラウザをリロード（`Ctrl + R`）してください
- データが消えた: ブラウザのキャッシュクリアで LocalStorage が削除された可能性。定期的なエクスポートを推奨

## 参照

- [ロードマップ](ROADMAP.md): 今後の機能予定
- [テーマガイド](THEMES.md): テーマカスタマイズ詳細
