...
- 監査タスク（TASK_039/040/041）の成果が **すべて `main / origin/main` に反映済み**であることを確認し、再開可能な形で申し送りを更新する。

### 調査/対応の確定事項（これまでの調査内容を含む）

- TASK_039（Embed SDK）
  - `same-origin`/`origin` 検証に関する挙動の整理を行い、主にエラーメッセージの誤解を招く文言を修正（後方互換性維持）。
  - PR #114 が `main` にマージ済み。
- TASK_040（Docs 整合監査）
  - `docs/GADGETS.md` などの「現行実装」と「提案/未実装」の混在を解消し、SSOT導線を明確化。
- TASK_041（smoke/dev-check 監査）
  - `scripts/dev-check.js` / smoke test の期待値と現行実装の整合を取り、読み手が保証範囲を誤認しない状態へ。

### 反映状況（main / origin/main）

- 上記を含む最新状態は `main` に集約済み。
...
- `git status -sb` が `## main...origin/main`（差分なし）であることを確認。
- `git diff --stat` が空（未コミット差分なし）であることを確認。

### 補足（Pre-flight観点）

- `npm run lint` は現状の `main` に対しても多数エラーで失敗（今回の申し送り追記が原因ではない）。
- そのため、再開時の確認は `npm run test:smoke`（= `scripts/dev-check.js`）を優先し、lintは別途タスクとして扱う。
