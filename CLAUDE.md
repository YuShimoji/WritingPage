...
直近の状態 (2026-03-18 session 8 handoff):
  - SP-078 読者プレビュー: 装飾パイプライン統合完了
    - CRITICAL修正: reader-preview.js に6段装飾パイプライン追加 (テキストボックス/テクスチャ/wikilink/傍点/ルビ/chapter://)
    - DSL退避方式: :::zw-* ブロックを markdown-it 前にプレースホルダー退避 → 処理後に復元 (extractDslBlocks/restoreDslBlocks)
    - editor-preview.js にも同じ DSL 退避方式を適用
    - exportHtml() に装飾CSS embed追加 (@keyframes/textbox/kenten/wikilink/ruby)
    - autoGroupChoices() 連動 (SP-072)
    - E2E 6→10件 (テクスチャ/傍点/wikilink/ルビ 4件追加)
    - 手動検証: モック3種作成 (ベケット/ボルヘス/クノー風)、textbox表示修正後の再確認待ち
  - SP-074 Web小説演出: partial/18%
  - SP-050 Story Wiki: partial/95%
  - SP-072 セクションリンク: done/100%
  - SP-016 拡張テキストボックス: done/100%
  - SP-060 装飾プリセット: done/100%
  - SP-078 読者プレビュー: done/100%
  - E2E: 390件/51ファイル (reader-preview 10件含む)
  - Lint: JS 0 errors
  - 残 partial: SP-074(18%), SP-050(95%), SP-012(70%), SP-005(75%)
