...
直近の状態 (2026-03-13):
  - SP-057 本文マイクロタイポグラフィ: done/100% (別セッション実装済み)
  - SP-052 Phase 2 軽量セクションコラプス: 実装完了 (90%)。+追加バグ修正済み (jumpToIndex追加)
  - SP-059 ルビ記法統一: {漢字|かな} 形式で実装済み
  - WYSIWYG を主軸、textarea はデバッグ用途
  - Outline途切れ/FocusMode重複: 856a320/866f768で修正済み
  - E2E: 262 tests, 260 passed / 1 failed / 1 skipped
  - E2E修正: writing-focus対応 (disableWritingFocus追加, 29→1件に削減)
  - E2E残失敗1件: editor-canvas-mode (betaEnabled:false, Canvas未完成のため対応不要)
  - plugin-manager修正: manifest id統一 + self-bootstrap化 (動的scriptロード順序問題解消)
  - 次: コラプスUX手動確認 / SP-059 Phase 2
