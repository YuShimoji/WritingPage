...
直近の状態:
  ...
  - session 40: WYSIWYG TB最適化 (13→11ボタン) + 装飾グループ/Canvas Mode 完全削除
  - UI モード切替は `window.ZenWriterApp.setUIMode/getUIMode` を優先利用する
  - サイドバー slim モード (`data-sidebar-slim`) でガジェット chrome が非表示
  - canonical docs は repo 内に作成済み (`docs/runtime-state.md`, `docs/project-context.md`, `docs/INVARIANTS.md` 等)
