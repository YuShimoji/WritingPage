...
Zen Writer: ブラウザベースの小説執筆エディタ (v0.3.30)。モジュラーガジェットシステム / JavaScript / Playwright E2E。
...
直近の状態 (2026-03-29 session 30):

- Session 30: SP-081 エディタ体験再構築 Phase 1-2
  - Phase 1 heading-based章管理を全削除(-254行)、chapterMode一本化
  - モード切替安定化: setUIModeにエッジホバーリセット+サイドバー自動閉/復元
  - フローティングツールバー: data-visible属性のみで状態管理統一
  - エッジホバーヒントテキスト追加(Focus モード、2回表示で自動消去)
  - ツールバー整理: Canvas Mode(beta)+装飾グループをhiddenで非表示
  - BP-5修正、ガジェットcleanup API、reader縦書きトグル
- E2E: 478 passed / 0 failed / 3 skipped (修正前: 80 failed)
- spec: done 42, partial 2 (SP-005/SP-081), removed 11, superseded 1
- 設計課題: Q1-Q4全解決。WP-001トリガー成立
...
| 2026-03-24 | SP-076 Phase 4: プリセットのみ、top/bottomドックはスコープ外 | プリセットのみ / bottom追加 / フルスコープ | top/bottomはユースケース不明+CSS再設計大。プリセットだけでSP-076をdone/100%にできる |
| 2026-03-24 | SP-076 Phase 4: プリセットUIはLoadoutManagerに統合 | LoadoutManager統合 / コマンドパレット / 専用ガジェット | 既存ロードアウトにdockLayout追加でガジェット+ドックを一括管理。UI変更最小 |
| 2026-03-24 | SectionsNavigatorをGADGETS.mdテーブルに追加(27→28件統一) | テーブル追加 / 分離記載維持 / 削除統合 | 実態と文書を一致させる最もシンプルな方法 |
| 2026-03-27 | WYSIWYG見出し/リスト: formatBlock + insertList に修正 | execCommand直接 / formatBlock | data-block値をexecCommandに直渡しは無効。Web標準コマンド名を使う |
| 2026-03-27 | textarea復帰: 「リッチテキストに戻る」バナー追加 | バナー / メインツールバー依存 | フォーカスモードでメインツールバーが隠れるため、モード内復帰UIが必須 |
| 2026-03-27 | SP-073 Phase 4: フリーハンド描画をRDP+ベジェ近似で実装 | 外部ライブラリ / 自作 | Catmull-Rom→CubicBezier変換。外部依存なし。simplifyRDP閾値3px |
| 2026-03-29 | SP-081: chapterModeを唯一の章管理方式に一本化 | chapterMode一本化 / 二重管理維持 | Phase 1 heading-basedとPhase 2 independent chaptersの二重管理がバグの温床。全ドキュメントをchapterMode自動適用 |
| 2026-03-29 | SP-081: フローティングツールバーの状態管理をdata-visible属性に統一 | data-visible属性のみ / JSプロパティ併用 | _floatingVisibleプロパティとDOM属性の三重管理がモード切替時の不整合原因 |
| 2026-03-29 | SP-081: エッジホバーヒントをFocusモードのみに限定 | Focus のみ / Focus+Blank / 全モード | Blankは完全非表示が目的。Normalはツールバー常時表示で不要 |
| 2026-03-29 | SP-081: 装飾グループ+Canvas ModeをメインツールバーからhiddenでNon表示 | hidden / 完全削除 / 維持 | WYSIWYGフローティングツールバーに移行済み。JS参照破壊を防ぐためhiddenで非表示化 |
