...
直近の状態 (2026-03-19 session 11+12):
  - SP-074 Phase 3補完完了: WYSIWYG round-trip対応 (.zw-typing/.zw-dialog → DSL serialization)
  - SP-074 Phase 4完了: スクロール連動演出 (:::zw-scroll{effect:"fade-in"} / slide-in / slide-up)
  - TextboxDslParser: scroll ブロック型追加 + effect/delay/threshold attr
  - TextboxEffectRenderer: renderScroll追加 + renderDialogにdata-dialog-*属性追加 (WYSIWYG round-trip用)
  - TextboxRichTextBridge.serializeHtml: .zw-typing/.zw-dialog → DSL変換追加
  - editor-wysiwyg.js: TBドロップダウンにタイピング/ダイアログ挿入ボタン追加 (_insertTypingBlock / _insertDialogBlock)
  - reader-preview.js: activateScrollTriggers (IntersectionObserver, threshold 0.2, delay対応, reduced-motion縮退)
  - editor-preview.js + reader-preview.js: DSL_BLOCK_RE に scroll 追加
  - CSS: .zw-scroll-trigger スタイル + fade-in/slide-in/slide-up + reduced-motion対応
  - E2E: typing-effect.spec.js に Phase 3+4 テスト追加 → 20件 passed
  - SP-050 リンク候補検出 (Phase 2 Step 4a): story-wiki.js に findUnlinkedMentions/showLinkCandidatesDialog/applyWikilinks追加
    CSS: .swiki-suggest-* / .swiki-dialog-actions実装済み
  - docs更新: spec-web-novel-effects.md (Phase 4 scroll構文), sp-074-phase-plan.md (Phase 4 done),
    spec-index.json (SP-074: 50%→75%), ROADMAP.md (SP-074: 75%), spec-story-wiki.md (Step 4a done)
  - Lint: JS 0 errors
  - 残 partial: SP-074(75%), SP-050(95%→Step4b形態素解析のみ残), SP-012(25%), SP-005(75%)
