...
直近の状態:
  ...
  - 文字表現の責務分離SSOTを新規整理 (`docs/specs/spec-text-expression-architecture.md`)
  - 装飾semantic preset、micro typography、ruby、日本語組版、Visual Profile Typography Pack などの仕様ドラフトが追加済み
  - WYSIWYG / decorations / textbox / canvas 向けの新規JSモジュールとE2E・unitテストが未コミット差分として存在
  - 執筆集中サイドバー、下部ナビ、サイドバー整理系の変更差分も引き続き混在している
  - Linux移行関連の一時作業は破棄済み（開発方針は既存環境継続）
  - 次: 文字表現 Tier 1 の実装境界を人間承認し、parser/serializer 正規化層と preset registry を固める
