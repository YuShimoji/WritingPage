    ...
    window.ZWGadgets.register('Wiki', function (root, api) {
      ...
      function renderMarkdownBasic(md) {
        try {
          ...
          html = html.replace(/\[\[([^\]]+)\]\]/g, function (match, content) {
            ...
            // 存在チェック (TASK_044)
            var exists = false;
            if (STORAGE && ensure(STORAGE.listWikiPages)) {
            ...
            return '<a href="' + href + '" class="wikilink' + brokenClass + '" data-wikilink="' + encodeURIComponent(link) + '">' + display + '</a>';
          });
          // 太字/斜体/インラインコード/リンク
      ...
      function updatePreview() {
        try {
          ...
          // バックリンクを取得して追加 (TASK_044)
          if (window.LinkGraph && ensure(window.LinkGraph.findBacklinks) && (state.currentId || title.value)) {
            var target = state.currentId || title.value;
            // タイトルでも検索対象にするため、LinkGraph側に配慮
            var backlinks = window.LinkGraph.findBacklinks(target);
            if (backlinks && backlinks.length > 0) {
              html += '<div class="wiki-backlinks">';
              html += '<h3>バックリンク</h3>';
              html += '<div class="wiki-backlinks-list">';
              backlinks.forEach(function (bl) {
                // bl.sourceTitle や bl.source を使用
                var label = bl.sourceTitle || bl.source || '不明なページ';
                var sid = bl.source || '';
                html += '<div class="wiki-backlink-item" data-id="' + sid + '" data-title="' + (bl.sourceTitle || '') + '">' + label + '</div>';
              });
              html += '</div></div>';
            }
          }
          ...
          // バックリンクのクリックイベント委譲
          var blItems = preview.querySelectorAll('.wiki-backlink-item');
          blItems.forEach(function (item) {
            item.addEventListener('click', function () {
              var id = item.dataset.id;
              if (id) loadToEditor(id);
            });
          });

          // Wikilink クリック委譲 (Wiki内遷移)
          var wlLinks = preview.querySelectorAll('.wikilink');
          wlLinks.forEach(function (link) {
            link.addEventListener('click', function (e) {
              e.preventDefault();
              var linkName = decodeURIComponent(link.dataset.wikilink || '');
              if (linkName) {
                // 名前で検索して開く
                var all = STORAGE.listWikiPages();
                var found = all.find(function (p) { return (p.title || '') === linkName; });
                if (found) {
                  loadToEditor(found.id);
                } else {
                  // 未作成ページの場合: 新規作成モードへ
      ...
      window.ZenWriterWikiAPI = {
        openPageByName: function (name) {
          if (!name) return;
          var all = STORAGE.listWikiPages();
          var found = all.find(function (p) { return (p.title || '') === name; });
          if (found) {
            loadToEditor(found.id);
            // Wikiガジェットを表示状態にする（可能であれば）
