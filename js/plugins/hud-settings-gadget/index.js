(function () {
    ...
    try {
      ...
      saveBtn.addEventListener('click', function () {
        var updatedHud = {
          ...
          duration: parseInt(durInput.value, 10) || 1200,
          bg: bgInput.value,
          fg: fgInput.value,
          opacity: parseFloat(opInput.value) || 0.75,
          message: msgInput.value.trim(),
          pinned: pinInput.checked,
          width: parseInt(widthInput.value, 10) || 240,
          fontSize: parseInt(fsInput.value, 10) || 14
        };
  ...
  window.ZWPlugin.register({
    id: 'hud-settings-gadget',
    name: 'HUD設定',
    version: '0.1.0',
    type: 'gadget',
    init: function (api) {
      api.gadgets.register('HUDSettings', renderHUDSettings, {
        groups: ['advanced'],
        title: (window.UILabels && window.UILabels.GADGET_HUD_TITLE) || 'HUD設定',
        description: 'HUDの位置・表示時間・見た目を調整。',
