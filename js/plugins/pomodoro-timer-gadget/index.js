...
(function () {
  ...
  window.ZWPlugin.register({
    id: 'pomodoro-timer-gadget',
    name: 'Pomodoro/集中タイマー',
    version: '0.1.0',
    type: 'gadget',
    init: function (api) {
   ...
   * Pomodoroタイマーガジェット
   */
  api.gadgets.register('PomodoroTimer', function (el, _api) {
    try {
    ...
    description: '作業と休憩のタイマーを切り替えて集中を維持。',
   ...
   * Pomodoro設定ガジェット
   */
  if (api.gadgets && typeof api.gadgets.registerSettings === 'function') {
    api.gadgets.registerSettings('PomodoroTimer', function (el, _ctx) {
    try {
      var timer = window.ZenWriterPomodoro;
      if (!timer) {
        el.textContent = (window.UILabels && window.UILabels.POMODORO_UNAVAILABLE) || 'タイマーが利用できません';
